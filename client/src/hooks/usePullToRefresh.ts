...
export const usePullToRefresh = ({
  ...
  const findScrollableAncestor = useCallback((target: HTMLElement | null) => {
  ...
  const handleTouchStart = useCallback(
    (e: TouchEvent) => {
      ...
      activeScrollContainerRef.current = scrollableAncestor;

      // Só ativar se o scroll real do contexto estiver no topo.
      const scrollTop = scrollableAncestor
    ...
    [enabled, findScrollableAncestor, isRefreshing]
  );
  ...
  const handleTouchMove = useCallback(
    (e: TouchEvent) => {
      ...
      const activeScrollContainer = activeScrollContainerRef.current;
      ...
      if (distance > 0) {
        ...
        const resistance =
          distance < threshold ? distance : threshold + (distance - threshold) * 0.3;
