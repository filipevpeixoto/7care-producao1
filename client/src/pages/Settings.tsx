...
export default function Settings() {
  ...
  const validateImportData = () => {
    ...
    const validatedData = importData.map((row, index) => {
      ...
      const validRoles = ['admin', 'superadmin', 'pastor', 'missionary', 'member', 'interested', 'diácono', 'membro', 'interessado'];
        if (!validRoles.some(role => tipoStr.includes(role))) {
        errors.push(`Linha ${lineNumber}: Tipo "${tipo}" não reconhecido. Use: Superadmin, Pastor, Missionary, Member ou Interested`);
          validatedRow.valid = false;
        }
      }
  ...
  const getRole = (tipo: string): string => {
    ...
    // Superadmin roles
    if (tipoLower.includes('superadmin') || 
