...
const connectionString = process.env.DATABASE_URL || 'postgresql://neondb_owner:npg_enihr4YBSDm8@ep-still-glade-ac5u1r48-pooler.sa-east-1.aws.neon.tech/neondb?sslmode=require';
...
async function migrateMeetingsTable() {
  ...
  try {
    ...
    // Verifica resultado
    const result = await sql`SELECT column_name FROM information_schema.columns WHERE table_name = 'meetings'`;
    console.log('Colunas atuais:', result.map((r: any) => r.column_name).join(', '));
