...
export const importRoutes = (app: Express): void => {
  ...
  app.post("/api/calendar/import-simple", upload.single('file'), async (req: Request, res: Response) => {
    try {
      ...
      for (let i = 0; i < data.length; i++) {
        try {
          ...
          if (Array.isArray(result)) {
            logger.info(`Evento de múltiplos dias inserido: ${eventTitle} (${result.length} dias)`);
