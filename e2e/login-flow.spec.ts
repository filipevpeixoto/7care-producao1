...
test.describe('Login Flow', () => {
  ...
  test('should render login page with all required elements', async ({ page }) => {
    ...
    const emailInput = page.locator(
      'input[type="email"], input[type="text"][name="email"], input[placeholder*="email" i], input[placeholder*="telefone" i]'
    );
  ...
  test('should show validation error for empty form submission', async ({ page }) => {
    ...
    const hasValidation = await page
      .locator('[role="alert"], .text-red, .text-destructive, :invalid, [data-state="open"]')
      .first()
  ...
  test('should show error for invalid credentials', async ({ page }) => {
    ...
    const errorMessage = page.locator(
      '[role="alert"], .toast, [data-state="open"], .text-red-500, .text-destructive'
    );
  ...
  test('should have working toggle between login and register', async ({ page }) => {
    ...
    const registerLink = page.locator(
      'button:has-text("Cadastre-se"), a:has-text("Cadastre-se"), button:has-text("Sign up"), button:has-text("Não tem conta")'
    );
    ...
    if (
      ...
      const registerForm = page.locator(
        'button:has-text("Criar conta"), button:has-text("Registrar"), button:has-text("Faça login"), input[placeholder*="nome" i]'
      );
